...
final class QuicClientTests {
    @Test func start() async throws {
        do {
            ...
            print(status)
            let message1 = try await quicClient.send(message: Data("Hello, World!".utf8), streamKind: .uniquePersistent)
            print("Client received 1: \(message1)")
            let message2 = try await quicClient.send(message: Data("Hello, swift!".utf8), streamKind: .uniquePersistent)
