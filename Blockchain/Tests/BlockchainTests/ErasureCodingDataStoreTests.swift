...
struct ErasureCodingDataStoreTests {
    ...
    func reconstructAuditBundleFromShards() async throws {
        ...
        let canReconstruct = try await dataStore.rocksdbStore.canReconstruct(erasureRoot: erasureRoot)
    ...
    func cleanupAuditEntries() async throws {
        ...
        try await dataStore.rocksdbStore.setTimestamp(erasureRoot: oldErasureRoot, timestamp: oldTimestamp)
    ...
    func cleanupD3LEntries() async throws {
        ...
        try await dataStore.rocksdbStore.setTimestamp(erasureRoot: erasureRoot, timestamp: oldTimestamp)
    ...
    func getStatisticsAfterOperations() async throws {
        ...
        // Get statistics
        let stats = try await dataStore.rocksdbStore.getStatistics()

        #expect(stats.auditCount == 1)
        #expect(stats.d3lCount == 1)
        #expect(stats.totalSegments == 10)
    }
}
